...
* **Radio Button Input** (`radioButtons()`): Present users with exclusive choices using radio buttons.

* **Select Input** (`selectInput()`): Offer users a dropdown menu to select from multiple options.

* **Date Input** (`dateInput()`): Allow users to input dates using a calendar widget or date picker.
...
* **Image Output** (`imageOutput()`): Render images dynamically, allowing for interactive exploration or presentation of visual content.
...
![](images/deploy.png){fig-alt="Screenshot of Rstudio" width="90%"}
...
![](images/shinylive.jpg){fig-alt="Shinylive diagram" width="70%"}
